...
V xMap<K, V>::put(K key, V value) {
  ...
  list.add(pEntry);
  this->count++;
  if (this->count > capacity * loadFactor) {
    // rehash(capacity * 2);
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K)) {
  ...
  for (auto pEntry : list) {
    if (keyEQ(pEntry->key, key)) {
      ...
      if(list.contains(pEntry)) {
        // cout << list.toString() << endl;
        deleteEntry(list.removeAt(list.indexOf(pEntry)));
        // cout << list.toString() << endl;
        // cout << this->table[index].toString() << endl;
      }
      this->count--;
      return retValue;
    }
